...
- **A action `submeterTemplate`, nesta ordem:** (1) resolve `criarScriptStoreDoServidor` + `criarTemplateStoreDoServidor`; papel vendedor → `sem_permissao`; (2) `buscar(scriptId)` — nulo → `script_nao_encontrado`; (3) `traduzirParaPosicional(conteudo)` — desconhecida → forward do código, **antes de qualquer IO externo**; (4) template existente com status `pending` → `falha('template_ja_pendente')` (re-submeter no meio de uma análise é recusado); (5) `criarDisparoServico().credencial(contaId)` — sem conexão → `sem_conexao_whatsapp`; (6) `nomeMetaDoTitulo(titulo, sufixo)` com sufixo aleatório curto do chamador; (7) `submeterTemplate` no Graph; (8) `criar` ou `substituir` no store com status devolvido (normalizado; a falsa devolve `approved` por default); (9) **só depois da gravação**, na re-submissão: `apagarTemplate` do nome antigo no Meta, best-effort — **falha não bloqueia nem desfaz nada** (nome novo nunca colide pelo sufixo; é higiene da WABA, e apagar ANTES deixaria um `approved` órfão no registro local se a submissão nova fosse recusada — achado de review da execução); (10) `revalidatePath('/scripts/' + scriptId)`.
