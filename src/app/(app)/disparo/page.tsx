...
import { criarTemplateStoreDoServidor, type TemplateWhatsApp } from '@/lib/data/templates'
import { mensagemDeErroScript } from '../scripts/erros'
import { ListaDeScripts } from '../scripts/lista'
import { Disparar, type ScriptParaDisparo } from './disparar'

/**
 * Os scripts para a area "Disparar" (Task 7): TODOS os da conta, nunca so' os
 * que a `lista` da biblioteca devolve — aquela ja saiu filtrada por
 * busca/tag/etapa da URL, e o Passo 1 do disparo precisa do catalogo inteiro
 * para deixar aprovado+atualizado selecionavel e mostrar o motivo dos outros.
 *
 * `dosScripts` e' UMA consulta para os N scripts, nao uma por item — mesma
 * forma de `templatesDoPainel` na ficha do lead. Degrada para `template: null`
 * em cada linha quando a leitura de template falha: sem template so' se perde
 * a selecao no Passo 1, e a area continua de pe (a mesma tolerancia do resto
 * desta pagina).
 */
async function scriptsParaDisparo(scripts: Script[]): Promise<ScriptParaDisparo[]> {
  if (scripts.length === 0) return []

  const porScript = new Map<string, TemplateWhatsApp>()
  const contexto = await criarTemplateStoreDoServidor()
  if (contexto.ok) {
    const templates = await contexto.valor.templates.dosScripts(scripts.map((s) => s.id))
    if (templates.ok) {
      for (const t of templates.valor) porScript.set(t.scriptId, t)
    }
  }
...
export default async function DisparoPage({
  ...
  const [lista, tagsDaConta, todosOsScripts] = await Promise.all([
    scripts.listar({ busca: busca || null, tag: tag || null, stageId: etapa || null }),
    scripts.tagsDaConta(),
    // Sem filtro nenhum: a area "Disparar" e o Passo 1 dela precisam do
    // catalogo inteiro da conta, nunca so' o recorte que `lista` aplica pelos
    // parametros da URL (ver o comentario de `scriptsParaDisparo` acima).
    scripts.listar({}),
  ])
  // Mesma tolerancia: sem as tags o <select> fica so com "Qualquer tag".
  const tags = tagsDaConta.ok ? tagsDaConta.valor : []
  // E sem o catalogo inteiro, a area de disparo fica vazia — nunca derruba a
  // pagina por uma segunda consulta de scripts que falhou.
  const paraDisparar = await scriptsParaDisparo(todosOsScripts.ok ? todosOsScripts.valor : [])
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
      ...
      {/* Regiao propria, e nao so um <div>: o Passo 1 de Disparar (acima) lista
          ...
          por texto de titulo (ou pelo estado vazio) acha as duas areas e vira
          ambiguidade tanto para teste quanto para leitor de tela. */}
      <div role="region" aria-label="Biblioteca de scripts" className="flex flex-col gap-4">
        <h2 className="text-lg font-semibold">Biblioteca</h2>

        <form method="get" action="/disparo" className="flex flex-wrap items-end gap-2">
